...
-- Pulse's own store.
--
-- Everything MSG91 owns is read-only to Pulse (`SELECT` and nothing else). This
-- schema is the other half: the six tables Pulse writes to, holding decisions,
-- drafts, timers, policy and outcomes.
--
-- It is a SEPARATE SCHEMA on purpose. The boundary between "MSG91's data, which
-- Pulse reads" and "Pulse's data, which Pulse owns" is a real one, and keeping
-- it at the schema level means moving to production is a connection string, not
-- a migration. Nothing in here has a foreign key into the legacy schema — the
-- account is referenced by user_pid as a plain value, because the two schemas
-- may not live on the same server for long.
--
--   apply:  npm run store:up
--   wipe:   npm run store:reset
-- ---------------------------------------------------------------------------

CREATE DATABASE IF NOT EXISTS pulse_store
